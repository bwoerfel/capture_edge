...
#define BUFFER_SIZE (1 << BUFFER_RING_BITS) // 2^10 =1024  // 2^9=512
...
void capture_edge_init(PIO pio, uint pin_base, float clk_div, uint irq) {
    ...
    // dma channel write pins -> 5 DMA CHANNEL
    // 1->2->3
    dma_channel_write_pins_ = dma_claim_unused_channel(true);   // war an 5.ter Stelle
    ...
    // [1]->2->3 dma_channel_write_pins
    dma_channel_config config_dma_channel_write_pins = dma_channel_get_default_config(dma_channel_write_pins_);
    channel_config_set_ring(&config_dma_channel_write_pins, true, BUFFER_RING_BITS+3);    // // war +3 Enable Wrapping to Write Adresses
    ...
    channel_config_set_dreq(&config_dma_channel_write_pins, pio_get_dreq(pio_, sm_, false));    // request data from PIO SM FIFO
    channel_config_set_chain_to(&config_dma_channel_write_pins, dma_channel_write_counter_);    // Channel to trigger when this channel completes.
    dma_channel_configure(dma_channel_write_pins_, &config_dma_channel_write_pins,
                          &buffer_pins_,                                                    // Inital write address
                          &pio_->rxf[sm_],                                                  // Initial read address
                          1, false);                                                        // Number of transfers to perform = 1, dont start yet

    // 1->[2]->3 dma_channel_counter_
    dma_channel_config config_dma_channel_write_counter = dma_channel_get_default_config(dma_channel_write_counter_);
    channel_config_set_ring(&config_dma_channel_write_counter, true, BUFFER_RING_BITS+1);     // war +1
    ...
    dma_channel_configure(dma_channel_write_counter_, &config_dma_channel_write_counter,
                          &buffer_counter_,                                     // Inital write address
                          &dma_hw->ch[dma_channel_counter_].transfer_count,     // Initial read address
                          1, false);                                            // Number of transfers to perform = 1, dont start yet

    // 1->2->[3] dma_channel_configure_write_pins_
    ...
    channel_config_set_ring(&config_dma_channel_configure_write_pins, true, BUFFER_RING_BITS+1);  // war +1
    ...
    dma_channel_configure(dma_channel_configure_write_pins_, &config_dma_channel_configure_write_pins,
                          &dma_hw->ch[dma_channel_write_pins_].al1_transfer_count_trig,     // Inital write address
                          &reload_write_pins_,                                              // Initial read address
                          1, false);                                                        // Number of transfers to perform = 1, dont start yet

    // [1]->2   config_dma_channel_counter
    ...
    channel_config_set_chain_to(&config_dma_channel_counter, dma_channel_reload_counter_);  // Channel to trigger when this channel completes
    dma_channel_configure(  dma_channel_counter_,           // DMA CHANNEL
                            &config_dma_channel_counter,    // -> DMA CHANNEL STRUCT
                            NULL,                           // Inital write address
                            NULL,                           // Initial read address
                            0xffffffff,                     // Number of transfers to perform
                            false);                         // True, to start immediatly

    // 1->[2]   config_dma_channel_reload_counter
    ...
    dma_channel_configure(dma_channel_reload_counter_, &config_dma_channel_reload_counter,
                          ...
                          1, false);                                                    // Number of transfers to perform
